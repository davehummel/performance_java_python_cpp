...
if __name__ == "__main__":
    ...
    t_cpu_only = timer(partial(run_experiment, n_iter, grid_shape, torch.device("cpu")))
    print(f"Runtime with CPU: {t_cpu_only:0.4f}s")

    print(f"Copy is {t_copy / t_vanilla:0.2f}x slower than vanilla")
    print(f"Inplace is {t_copy / t_inplace:0.2f}x faster than copy")
    print(f"CPU is {t_cpu_only / t_vanilla:0.2f}x slower than vanilla")